...
def generate_configs(hw: Dict, mac_strategy: Optional[str], selected_models: List[str], data_dir: str):
    print(f"\n{Color.CYAN}⚙️  Generating customized environment and configuration files...{Color.ENDC}")
    
    # 1. Generate .env file
    env_content = []
    
    db_password = os.getenv("DB_PASSWORD", "webui-secure-password-" + str(int(time.time()))[-4:])
    port = "8080"
    ollama_port = "11434"
    webui_secret = os.getenv("OPEN_WEBUI_SECRET_KEY", "secret-" + str(int(time.time())))
...
def main():
    ...
    generate_configs(hw, mac_strategy, selected_models, data_dir)
