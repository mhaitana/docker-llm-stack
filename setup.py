...
def generate_configs(hw: Dict, mac_strategy: Optional[str], openclaw_strategy: str, selected_models: List[str], data_dir: str, openclaw_token: str):
    ...
    env_content.append(f"OPENCLAW_STRATEGY={openclaw_strategy}")
    if mac_strategy:
        env_content.append(f"MAC_STRATEGY={mac_strategy}")
    env_content.append(f"SELECTED_MODELS={','.join(selected_models)}")
    ...
    try:
        ...
        # Prepare runtime dashboard directory
        runtime_dashboard_dir = os.path.join(resolved_data_dir, "dashboard")
        ...
        # Inject spec values into the runtime index.html
        runtime_index_html = os.path.join(runtime_dashboard_dir, "index.html")
        if os.path.exists(runtime_index_html):
            with open(runtime_index_html, "r") as f:
                html_content = f.read()
                
            os_display = hw['os']
            if os_display == 'Darwin':
                os_display = 'macOS'
            elif os_display == 'Linux':
                os_display = 'Linux'
            elif os_display == 'Windows':
                os_display = 'Windows'
                
            gpu_display = hw['gpu_name'] if hw['gpu_name'] else 'None (CPU Only)'
            gpu_vram_display = f"{hw['gpu_vram_gb']:.1f}" if hw['gpu_vram_gb'] > 0 else '0.0'
            
            html_content = html_content.replace("{{OS_NAME}}", os_display)
            html_content = html_content.replace("{{CPU_NAME}}", hw['cpu_str'])
            html_content = html_content.replace("{{CPU_CORES}}", str(hw['cpu_cores']))
            html_content = html_content.replace("{{RAM_GB}}", f"{hw['ram_gb']:.1f}")
            html_content = html_content.replace("{{GPU_NAME}}", gpu_display)
            html_content = html_content.replace("{{GPU_VRAM}}", gpu_vram_display)
            html_content = html_content.replace("{{DATA_DIR}}", data_dir)
            html_content = html_content.replace("{{OPENCLAW_TOKEN}}", openclaw_token)
            
            with open(runtime_index_html, "w") as f:
                f.write(html_content)
            print(f"  ✅ Rendered dashboard template to: {Color.BOLD}{runtime_index_html}{Color.ENDC}")
...
def main():
    ...
    # 2. Dependency Check
    if not headless and not check_dependencies():
        sys.exit(1)
    ...
    if hw['os'] == 'Darwin':
        if headless:
            mac_strategy = existing_env.get("MAC_STRATEGY") or "native"
        else:
            mac_strategy = setup_mac_options(hw)
        
    # 3b. OpenClaw execution mode check
    if headless:
        openclaw_strategy = existing_env.get("OPENCLAW_STRATEGY") or "docker"
    else:
        openclaw_strategy = setup_openclaw_options(hw)
        
    # 4. Model Selection
    if headless:
        selected_models_str = existing_env.get("SELECTED_MODELS", "")
    ...
    if headless:
        data_dir = existing_env.get("DATA_DIR") or "./data"
    else:
        data_dir = "./data"
    ...
    if generate_only:
        print(f"\n{Color.GREEN}✅ Configuration files generated successfully. Skipping container launch and model pulling.{Color.ENDC}")
        return
    ...
    print(f"============================================================")
    print(f"🏠  {Color.BOLD}Dashboard Landing Page:{Color.ENDC} http://localhost:{dashboard_port}")
